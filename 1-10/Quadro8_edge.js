/**
 * Adobe Edge: symbol definitions
 */
(function($, Edge, compId){
//images folder
var im='images/';

var fonts = {};


var resources = [
];
var symbols = {
"stage": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
         dom: [
         {
            id:'G-Cielo',
            type:'image',
            rect:['-111px','-15px','1300px','647px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"G-Cielo.png",'0px','0px']
         },
         {
            id:'NUVOLA',
            type:'rect',
            rect:['247px','38px','auto','auto','auto','auto'],
            opacity:0.95,
            userClass:"Nuvola"
         },
         {
            id:'Q7-Q8-pratobg',
            type:'image',
            rect:['-45px','82px','1154px','204px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q7-Q8-pratobg.png",'0px','0px']
         },
         {
            id:'Q7-Q8-prato-front',
            type:'image',
            rect:['-40px','207px','1144px','566px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q7-Q8-prato-front.png",'0px','0px']
         },
         {
            id:'Q8-lago',
            type:'image',
            rect:['-56px','329px','906px','406px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-lago.png",'0px','0px'],
            transform:[[],[],[],['0.97537','0.97537']]
         },
         {
            id:'Q8-rocciafronde',
            type:'image',
            rect:['443px','251px','140px','139px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-rocciafronde.png",'0px','0px']
         },
         {
            id:'Q8-ninfea1',
            type:'image',
            rect:['570px','350px','69px','44px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-ninfea1.png",'0px','0px']
         },
         {
            id:'Q8-loto22',
            type:'image',
            rect:['585px','352px','40px','32px','auto','auto'],
            clip:['rect(0px 40px 26.349365234375px 0px)'],
            fill:["rgba(0,0,0,0)",im+"Q8-loto2.png",'0px','0px']
         },
         {
            id:'Q8-loto1',
            type:'image',
            rect:['639px','355px','42px','34px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-loto1.png",'0px','0px']
         },
         {
            id:'Q8-frondematte',
            type:'image',
            rect:['-65px','-15px','651px','626px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-frondematte.png",'0px','0px'],
            transform:[[],[],[],['0.85623','0.85623']]
         },
         {
            id:'Q8-fronde4',
            type:'image',
            rect:['220px','-1px','105px','620px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-fronde4.png",'0px','0px']
         },
         {
            id:'giunco',
            type:'rect',
            rect:['220','114px','auto','auto','auto','auto']
         },
         {
            id:'giunco3',
            type:'rect',
            rect:['126px','90px','auto','auto','auto','auto']
         },
         {
            id:'giunco2',
            type:'rect',
            rect:['220','85px','auto','auto','auto','auto']
         },
         {
            id:'paperatutto6',
            type:'rect',
            rect:['83','559px','auto','auto','auto','auto'],
            userClass:"paperatutto6"
         },
         {
            id:'paperatutto7',
            type:'rect',
            rect:['83','559px','auto','auto','auto','auto'],
            userClass:"paperatutto7"
         },
         {
            id:'paperatutto5',
            type:'rect',
            rect:['83','559px','auto','auto','auto','auto'],
            userClass:"paperatutto5"
         },
         {
            id:'Q8-fronda',
            type:'image',
            rect:['418px','161px','80px','458px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-fronda.png",'0px','0px']
         },
         {
            id:'paperatutto3',
            type:'rect',
            rect:['83','559px','auto','auto','auto','auto'],
            userClass:"paperatutto3"
         },
         {
            id:'paperatutto1',
            type:'rect',
            rect:['83','559px','auto','auto','auto','auto'],
            userClass:"paperatutto1"
         },
         {
            id:'paperatutto2',
            type:'rect',
            rect:['83','559px','auto','auto','auto','auto'],
            userClass:"paperatutto2"
         },
         {
            id:'paperatutto4',
            type:'rect',
            rect:['83','559px','auto','auto','auto','auto'],
            userClass:"paperatutto4"
         },
         {
            id:'paperatutto8',
            type:'rect',
            rect:['83','559px','auto','auto','auto','auto'],
            userClass:"paperatutto8"
         },
         {
            id:'Scoiattolo',
            type:'rect',
            rect:['640px','267px','auto','auto','auto','auto'],
            transform:[[],[],[],['0.82481','0.82481']]
         },
         {
            id:'trigger-coda',
            type:'rect',
            rect:['897px','286px','140px','197px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"]
         },
         {
            id:'Q8-fronde3',
            type:'image',
            rect:['319px','28px','199px','648px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-fronde3.png",'0px','0px']
         },
         {
            id:'Q8-fronde2',
            type:'image',
            rect:['-71px','75px','250px','622px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-fronde2.png",'0px','0px']
         },
         {
            id:'Q8-fronde1',
            type:'image',
            rect:['36px','-7px','340px','691px','auto','auto'],
            fill:["rgba(0,0,0,0)",im+"Q8-fronde1.png",'0px','0px']
         },
         {
            id:'waterdue',
            type:'rect',
            rect:['532','578','auto','auto','auto','auto'],
            transform:[[],[],[],['1.14634','1.14634']]
         },
         {
            id:'watertre',
            type:'rect',
            rect:['-13px','729px','auto','auto','auto','auto']
         },
         {
            id:'wateruno',
            type:'rect',
            rect:['304px','705px','auto','auto','auto','auto']
         },
         {
            id:'giunco4',
            type:'rect',
            rect:['44px','158px','auto','auto','auto','auto'],
            transform:[[],['-6']]
         },
         {
            id:'trigduck8',
            type:'rect',
            rect:['42px','524px','140px','108px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"],
            userClass:"trigduck8"
         },
         {
            id:'trigduck7',
            type:'rect',
            rect:['42px','524px','140px','108px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"],
            userClass:"trigduck7"
         },
         {
            id:'trigduck6',
            type:'rect',
            rect:['42px','524px','140px','108px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"],
            userClass:"trigduck6"
         },
         {
            id:'trigduck5',
            type:'rect',
            rect:['42px','524px','140px','108px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"],
            userClass:"trigduck5"
         },
         {
            id:'trigduck4',
            type:'rect',
            rect:['42px','524px','140px','108px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"],
            userClass:"trigduck4"
         },
         {
            id:'trigduck3',
            type:'rect',
            rect:['42px','524px','140px','108px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"],
            userClass:"trigduck3"
         },
         {
            id:'trigduck2',
            type:'rect',
            rect:['42px','524px','140px','108px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"],
            userClass:"trigduck2"
         },
         {
            id:'trigduck1',
            type:'rect',
            rect:['42px','524px','140px','108px','auto','auto'],
            fill:["rgba(192,192,192,1)"],
            stroke:[0,"rgba(0,0,0,1)","none"],
            userClass:"trigduck1"
         },
         {
            id:'numerazio',
            type:'rect',
            rect:['54','30','auto','auto','auto','auto']
         },
         {
            id:'FUMELLO',
            type:'rect',
            rect:['521px','-232px','auto','auto','auto','auto']
         },
         {
            id:'Menu',
            type:'rect',
            rect:['-67','697px','auto','auto','auto','auto'],
            userClass:"menu"
         }],
         symbolInstances: [
         {
            id:'paperatutto2',
            symbolName:'paperatutto2'
         },
         {
            id:'giunco2',
            symbolName:'giunco'
         },
         {
            id:'paperatutto4',
            symbolName:'paperatutto4'
         },
         {
            id:'giunco3',
            symbolName:'giunco'
         },
         {
            id:'paperatutto6',
            symbolName:'paperatutto6'
         },
         {
            id:'paperatutto7',
            symbolName:'paperatutto7'
         },
         {
            id:'Menu',
            symbolName:'Menu'
         },
         {
            id:'paperatutto8',
            symbolName:'paperatutto8'
         },
         {
            id:'paperatutto3',
            symbolName:'paperatutto3'
         },
         {
            id:'watertre',
            symbolName:'fronde3WATER'
         },
         {
            id:'giunco',
            symbolName:'giunco'
         },
         {
            id:'paperatutto5',
            symbolName:'paperatutto5'
         },
         {
            id:'Scoiattolo',
            symbolName:'Scoiattolo'
         },
         {
            id:'waterdue',
            symbolName:'fronde3WATER'
         },
         {
            id:'wateruno',
            symbolName:'fronde2WATER'
         },
         {
            id:'NUVOLA',
            symbolName:'NUVOLA'
         },
         {
            id:'paperatutto1',
            symbolName:'paperatutto1'
         },
         {
            id:'numerazio',
            symbolName:'numerazio'
         },
         {
            id:'giunco4',
            symbolName:'giunco'
         },
         {
            id:'FUMELLO',
            symbolName:'FUMELLO'
         }
         ]
      },
   states: {
      "Base State": {
         "${_paperatutto7}": [
            ["style", "top", '559px']
         ],
         "${_Q8-fronde3}": [
            ["style", "left", '319px'],
            ["style", "top", '28px']
         ],
         "${_trigduck3}": [
            ["style", "top", '524px'],
            ["style", "opacity", '0'],
            ["style", "left", '42px']
         ],
         "${_trigger-coda}": [
            ["style", "top", '286px'],
            ["style", "opacity", '0'],
            ["style", "left", '897px']
         ],
         "${_Scoiattolo}": [
            ["style", "top", '267px'],
            ["transform", "scaleX", '0.82481'],
            ["transform", "scaleY", '0.82481'],
            ["style", "left", '640px']
         ],
         "${_Q8-fronde2}": [
            ["style", "left", '-72px'],
            ["style", "top", '36px']
         ],
         "${_Q7-Q8-pratobg}": [
            ["style", "left", '-45px'],
            ["style", "top", '82px']
         ],
         "${_Q8-frondematte}": [
            ["style", "top", '-15px'],
            ["transform", "scaleX", '0.85623'],
            ["transform", "scaleY", '0.85623'],
            ["style", "left", '-65px']
         ],
         "${_waterdue}": [
            ["style", "top", '711px'],
            ["transform", "scaleY", '1.14634'],
            ["style", "left", '119px'],
            ["transform", "scaleX", '1.14634']
         ],
         "${_FUMELLO}": [
            ["style", "top", '-232px'],
            ["style", "left", '521px']
         ],
         "${_paperatutto1}": [
            ["style", "top", '559px']
         ],
         "${_wateruno}": [
            ["style", "left", '304px'],
            ["style", "top", '705px']
         ],
         "${_watertre}": [
            ["style", "top", '720px'],
            ["transform", "scaleY", '1.7561'],
            ["style", "left", '-19px'],
            ["transform", "scaleX", '1.7561']
         ],
         "${_trigduck7}": [
            ["style", "top", '524px'],
            ["style", "opacity", '0'],
            ["style", "left", '42px']
         ],
         "${_giunco2}": [
            ["style", "left", '399px'],
            ["style", "top", '85px']
         ],
         "${_trigduck5}": [
            ["style", "top", '524px'],
            ["style", "opacity", '0'],
            ["style", "left", '42px']
         ],
         "${_trigduck4}": [
            ["style", "top", '524px'],
            ["style", "opacity", '0'],
            ["style", "left", '42px']
         ],
         "${_trigduck6}": [
            ["style", "top", '524px'],
            ["style", "opacity", '0'],
            ["style", "left", '42px']
         ],
         "${_giunco4}": [
            ["style", "-webkit-transform-origin", [22.19,97.98], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [22.19,97.98],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [22.19,97.98],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [22.19,97.98],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [22.19,97.98],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "top", '158px'],
            ["style", "left", '44px'],
            ["transform", "rotateZ", '-6deg']
         ],
         "${_NUVOLA}": [
            ["style", "top", '-57px'],
            ["style", "opacity", '0.95'],
            ["style", "left", '-18px']
         ],
         "${_Q8-rocciafronde}": [
            ["style", "left", '443px'],
            ["style", "top", '251px']
         ],
         "${_paperatutto5}": [
            ["style", "top", '559px']
         ],
         "${_Q8-lago}": [
            ["style", "top", '329px'],
            ["transform", "scaleY", '0.97537'],
            ["style", "height", '406px'],
            ["transform", "scaleX", '0.97537'],
            ["style", "left", '-56px'],
            ["style", "width", '906px']
         ],
         "${_trigduck8}": [
            ["style", "top", '524px'],
            ["style", "opacity", '0'],
            ["style", "left", '42px']
         ],
         "${_G-Cielo}": [
            ["style", "left", '-111px'],
            ["style", "top", '-15px']
         ],
         "${_trigduck2}": [
            ["style", "top", '524px'],
            ["style", "opacity", '0'],
            ["style", "left", '42px']
         ],
         "${_Q8-fronde1}": [
            ["style", "left", '36px'],
            ["style", "top", '-7px']
         ],
         "${_paperatutto4}": [
            ["style", "top", '559px']
         ],
         "${_paperatutto3}": [
            ["style", "top", '559px']
         ],
         "${_giunco3}": [
            ["style", "top", '90px'],
            ["style", "left", '126px'],
            ["transform", "scaleX", '-1']
         ],
         "${_paperatutto8}": [
            ["style", "top", '559px']
         ],
         "${_trigduck1}": [
            ["style", "top", '524px'],
            ["style", "opacity", '0'],
            ["style", "left", '42px']
         ],
         "${_paperatutto6}": [
            ["style", "top", '559px']
         ],
         "${_paperatutto2}": [
            ["style", "top", '559px']
         ],
         "${_Q8-fronda}": [
            ["style", "left", '418px'],
            ["style", "top", '161px']
         ],
         "${_Q7-Q8-prato-front}": [
            ["style", "left", '-40px'],
            ["style", "top", '207px']
         ],
         "${_Q8-ninfea1}": [
            ["style", "left", '570px'],
            ["style", "top", '350px']
         ],
         "${_Menu}": [
            ["style", "left", '-68px'],
            ["style", "top", '697px']
         ],
         "${_Q8-loto22}": [
            ["style", "top", '352px'],
            ["style", "left", '585px'],
            ["style", "clip", [0,40,26.349365234375,0], {valueTemplate:'rect(@@0@@px @@1@@px @@2@@px @@3@@px)'} ]
         ],
         "${_Stage}": [
            ["color", "background-color", 'rgba(255,255,255,1)'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '768px'],
            ["style", "width", '1024px']
         ],
         "${_Q8-loto1}": [
            ["style", "left", '639px'],
            ["style", "top", '355px']
         ],
         "${_giunco}": [
            ["style", "top", '114px']
         ],
         "${_Q8-fronde4}": [
            ["style", "left", '220px'],
            ["style", "top", '-1px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 0,
         autoPlay: true,
         timeline: [
            { id: "eid4628", tween: [ "style", "${_watertre}", "top", '720px', { fromValue: '720px'}], position: 0, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4629", tween: [ "style", "${_watertre}", "left", '-19px', { fromValue: '-19px'}], position: 0, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4495", tween: [ "style", "${_Q8-fronde2}", "left", '-72px', { fromValue: '-72px'}], position: 0, duration: 0 },
            { id: "eid4519", tween: [ "style", "${_Q8-fronde2}", "top", '36px', { fromValue: '36px'}], position: 0, duration: 0 },
            { id: "eid3874", trigger: [ function executeSymbolFunction(e, data) { this._executeSymbolAction(e, data); }, ['play', '${_NUVOLA}', [] ], ""], position: 0 }         ]
      }
   }
},
"numerazio": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'numerigruppo',
      type: 'group',
      rect: ['0','0','350','281','auto','auto'],
      c: [
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-1',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum1.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-2',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum2.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-3',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum3.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-4',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum4.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-5',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum5.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-6',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum6.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-7',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum7.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-8',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum8.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-9',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum9.svg','0px','0px']
      },
      {
         rect: ['0px','0px','350px','281px','auto','auto'],
         id: 'Num-10',
         userClass: 'numeris',
         type: 'image',
         fill: ['rgba(0,0,0,0)','images/svgNum10.svg','0px','0px']
      }]
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_numerigruppo}": [
            ["transform", "scaleY", '0'],
            ["transform", "rotateZ", '-180deg'],
            ["motion", "location", '175px 140.5px'],
            ["style", "opacity", '0'],
            ["transform", "scaleX", '0']
         ],
         "${_Num-3}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${_Num-1}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${_Num-9}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${_Num-4}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${_Num-5}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${_Num-7}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${_Num-10}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${symbolSelector}": [
            ["style", "height", '281px'],
            ["style", "width", '350px']
         ],
         "${_Num-6}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${_Num-2}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ],
         "${_Num-8}": [
            ["style", "top", '0px'],
            ["style", "left", '0px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 2427,
         autoPlay: false,
         timeline: [
            { id: "eid2424", tween: [ "transform", "${_numerigruppo}", "rotateZ", '0deg', { fromValue: '-180deg'}], position: 0, duration: 786, easing: "swing" },
            { id: "eid2435", tween: [ "transform", "${_numerigruppo}", "rotateZ", '0deg', { fromValue: '0deg'}], position: 1884, duration: 0, easing: "swing" },
            { id: "eid2431", tween: [ "style", "${_numerigruppo}", "opacity", '1', { fromValue: '0'}], position: 0, duration: 440, easing: "swing" },
            { id: "eid2434", tween: [ "style", "${_numerigruppo}", "opacity", '1', { fromValue: '1'}], position: 1884, duration: 0, easing: "swing" },
            { id: "eid2428", tween: [ "transform", "${_numerigruppo}", "scaleX", '0.9', { fromValue: '0'}], position: 0, duration: 786, easing: "swing" },
            { id: "eid2439", tween: [ "transform", "${_numerigruppo}", "scaleX", '0', { fromValue: '0.9'}], position: 1884, duration: 543 },
            { id: "eid2422", tween: [ "motion", "${_numerigruppo}", [[23.77,256.34,0,0],[414.45,85.73,421.02,22.34,335.45,17.8],[572.37,120.92,0,0]]], position: 0, duration: 786, easing: "swing" },
            { id: "eid2437", tween: [ "motion", "${_numerigruppo}", [[572.5,120.59,0,0],[823.85,251,-142.86,693.1]]], position: 1884, duration: 274 },
            { id: "eid4496", tween: [ "motion", "${_numerigruppo}", [[572.37,120.92,0,0],[823.85,251,0,0]]], position: 2158, duration: 0 },
            { id: "eid2438", tween: [ "motion", "${_numerigruppo}", [[823.85,251,-67.92,329.52],[228.19,964.99,0,0]]], position: 2158, duration: 269 },
            { id: "eid2429", tween: [ "transform", "${_numerigruppo}", "scaleY", '0.9', { fromValue: '0'}], position: 0, duration: 786, easing: "swing" },
            { id: "eid2440", tween: [ "transform", "${_numerigruppo}", "scaleY", '0', { fromValue: '0.9'}], position: 1884, duration: 543 }         ]
      }
   }
},
"play": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'Q0-Play',
      type: 'image',
      rect: ['0px','0px','80px','80px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/svg-play.svg','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_Q0-Play}": [
            ["style", "top", '0px'],
            ["transform", "scaleY", '1'],
            ["transform", "scaleX", '1'],
            ["style", "left", '0px'],
            ["subproperty", "filter.saturate", '1']
         ],
         "${symbolSelector}": [
            ["style", "height", '80px'],
            ["style", "width", '80px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 393,
         autoPlay: false,
         timeline: [
            { id: "eid11553", tween: [ "transform", "${_Q0-Play}", "scaleX", '0.8', { fromValue: '1'}], position: 0, duration: 223, easing: "swing" },
            { id: "eid11554", tween: [ "transform", "${_Q0-Play}", "scaleX", '1', { fromValue: '0.8'}], position: 223, duration: 170, easing: "swing" },
            { id: "eid11551", tween: [ "transform", "${_Q0-Play}", "scaleY", '0.8', { fromValue: '1'}], position: 0, duration: 223, easing: "swing" },
            { id: "eid11552", tween: [ "transform", "${_Q0-Play}", "scaleY", '1', { fromValue: '0.8'}], position: 223, duration: 170, easing: "swing" }         ]
      }
   }
},
"FUMELLO": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      userClass: 'fumello',
      rect: ['0px','0px','446px','287px','auto','auto'],
      id: 'Fumello',
      opacity: 1,
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/Fumello-Q8.png','0px','0px','445.97px','286.67px']
   },
   {
      userClass: 'fume',
      rect: ['-8px','172px','466px','287px','auto','auto'],
      id: 'Rectangle',
      stroke: [0,'rgba(0,0,0,1)','none'],
      type: 'rect',
      fill: ['rgba(192,192,192,1)']
   },
   {
      type: 'rect',
      id: 'play',
      rect: ['333px','140px','auto','auto','auto','auto'],
      userClass: 'playvoce',
      transform: [[0,0],[],[],['0.84','0.84']]
   }],
   symbolInstances: [
   {
      id: 'play',
      symbolName: 'play'
   }   ]
   },
   states: {
      "Base State": {
         "${_Fumello}": [
            ["style", "top", '-8px'],
            ["style", "height", '287px'],
            ["style", "opacity", '1'],
            ["style", "left", '0px'],
            ["style", "width", '446px']
         ],
         "${_Rectangle}": [
            ["style", "top", '-69px'],
            ["style", "opacity", '0'],
            ["style", "-webkit-transform-origin", [50,91], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [50,91],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [50,91],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [50,91],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [50,91],{valueTemplate:'@@0@@% @@1@@%'}]
         ],
         "${_play}": [
            ["style", "-webkit-transform-origin", [50,50], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [50,50],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [50,50],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [50,50],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [50,50],{valueTemplate:'@@0@@% @@1@@%'}],
            ["transform", "scaleY", '0.84'],
            ["transform", "scaleX", '0.84'],
            ["style", "left", '333px'],
            ["style", "top", '136px']
         ],
         "${symbolSelector}": [
            ["style", "height", '287px'],
            ["style", "width", '446px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 1500,
         autoPlay: false,
         labels: {
            "ENTRATA": 0,
            "USCITA": 1000
         },
         timeline: [
            { id: "eid6395", tween: [ "style", "${_play}", "top", '329px', { fromValue: '136px'}], position: 0, duration: 484, easing: "easeOutBack" },
            { id: "eid6396", tween: [ "style", "${_play}", "top", '140px', { fromValue: '329px'}], position: 1000, duration: 500, easing: "easeOutBack" },
            { id: "eid96", tween: [ "style", "${_Rectangle}", "top", '172px', { fromValue: '-69px'}], position: 0, duration: 484, easing: "easeOutBack" },
            { id: "eid511", tween: [ "style", "${_Rectangle}", "top", '-65px', { fromValue: '172px'}], position: 1000, duration: 500, easing: "easeOutBack" },
            { id: "eid93", tween: [ "style", "${_Fumello}", "top", '177px', { fromValue: '-8px'}], position: 0, duration: 484, easing: "easeOutBack" },
            { id: "eid95", tween: [ "style", "${_Fumello}", "top", '0px', { fromValue: '177px'}], position: 1000, duration: 500, easing: "easeOutBack" }         ]
      }
   }
},
"MENUz": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      rect: ['-50px','12px','1260px','235px','auto','auto'],
      id: 'Fondo-Menu',
      opacity: 0.7,
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/G-Menu.png','0px','0px']
   },
   {
      id: 'HOME_butt',
      type: 'rect',
      userClass: 'home',
      rect: ['55px','72px','auto','auto','auto','auto']
   },
   {
      rect: ['1033px','82px','auto','auto','auto','auto'],
      id: 'NEXT',
      transform: [[0,0],[],[],['0.58314','0.58314']],
      type: 'rect',
      userClass: 'next'
   },
   {
      rect: ['17.6%','30.3%','90px','70px','auto','auto'],
      userClass: 'fumnum1',
      id: 'Numero_1',
      opacity: 1,
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum1.svg','0px','0px']
   },
   {
      rect: ['24.5%','30.3%','90px','70px','auto','auto'],
      userClass: 'fumnum2',
      id: 'Numero_2',
      opacity: 1,
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum2.svg','0px','0px']
   },
   {
      rect: ['31.3%','30.3%','90px','70px','auto','auto'],
      userClass: 'fumnum3',
      id: 'Numero_3',
      opacity: 1,
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum3.svg','0px','0px']
   },
   {
      rect: ['38.1%','30.3%','90px','70px','auto','auto'],
      userClass: 'fumnum4',
      id: 'Numero_4',
      opacity: 1,
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum4.svg','0px','0px']
   },
   {
      rect: ['45.1%','30.3%','90px','70px','auto','auto'],
      userClass: 'fumnum5',
      id: 'Numero_5',
      opacity: 1,
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum5.svg','0px','0px']
   },
   {
      rect: ['606px','72px','90px','70px','auto','auto'],
      id: 'Num-6',
      userClass: 'fumnum6',
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum6.svg','0px','0px']
   },
   {
      rect: ['689px','72px','90px','70px','auto','auto'],
      id: 'Num-7',
      userClass: 'fumnum7',
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum7.svg','0px','0px']
   },
   {
      rect: ['773px','72px','90px','70px','auto','auto'],
      id: 'Num-8',
      userClass: 'fumnum8',
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum8.svg','0px','0px']
   },
   {
      rect: ['856px','72px','90px','70px','auto','auto'],
      id: 'Num-9',
      userClass: 'fumnum9',
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum9.svg','0px','0px']
   },
   {
      rect: ['938px','72px','90px','70px','auto','auto'],
      id: 'Num-10',
      userClass: 'fumnum10',
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svgBWNum10.svg','0px','0px']
   },
   {
      rect: ['76px','129px','auto','auto','auto','auto'],
      id: 'musica',
      transform: [[0,0],[],[],['0.65','0.65']],
      type: 'rect',
      userClass: 'musica'
   },
   {
      id: 'Testina-Menu',
      type: 'rect',
      userClass: 'testina',
      rect: ['1013','0','auto','auto','auto','auto']
   },
   {
      rect: ['65px','7px','auto','auto','auto','auto'],
      id: 'reload',
      transform: [[0,0],[],[],['0.65','0.65']],
      type: 'rect',
      userClass: 'reload'
   },
   {
      rect: ['147px','82px','auto','auto','auto','auto'],
      id: 'PREV2',
      transform: [[0,0],[],[],['0.58929','0.57999']],
      type: 'rect',
      userClass: 'prev'
   }],
   symbolInstances: [
   {
      id: 'Testina-Menu',
      symbolName: 'testina'
   },
   {
      id: 'musica',
      symbolName: 'musica'
   },
   {
      id: 'PREV2',
      symbolName: 'PREV'
   },
   {
      id: 'NEXT',
      symbolName: 'NEXT'
   },
   {
      id: 'reload',
      symbolName: 'reload'
   },
   {
      id: 'HOME_butt',
      symbolName: 'HOME_butt'
   }   ]
   },
   states: {
      "Base State": {
         "${_reload}": [
            ["transform", "scaleX", '0.65'],
            ["style", "left", '109px'],
            ["transform", "scaleY", '0.65'],
            ["style", "top", '5px']
         ],
         "${_Num-9}": [
            ["style", "height", '70px'],
            ["style", "top", '72px'],
            ["style", "left", '856px'],
            ["style", "width", '90px']
         ],
         "${_musica}": [
            ["transform", "scaleX", '0.65'],
            ["style", "left", '60px'],
            ["transform", "scaleY", '0.65'],
            ["style", "top", '5px']
         ],
         "${_Num-7}": [
            ["style", "height", '70px'],
            ["style", "top", '72px'],
            ["style", "left", '689px'],
            ["style", "width", '90px']
         ],
         "${_Num-10}": [
            ["style", "height", '70px'],
            ["style", "top", '72px'],
            ["style", "left", '938px'],
            ["style", "width", '90px']
         ],
         "${_Fondo-Menu}": [
            ["style", "top", '12px'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '235px'],
            ["style", "opacity", '0.7'],
            ["style", "left", '-50px'],
            ["style", "width", '1260px']
         ],
         "${_Numero_4}": [
            ["style", "top", '30.25%'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '70px'],
            ["style", "opacity", '1'],
            ["style", "left", '38.1%'],
            ["style", "width", '90px']
         ],
         "${_Numero_5}": [
            ["style", "top", '30.25%'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '70px'],
            ["style", "opacity", '1'],
            ["style", "left", '45.11%'],
            ["style", "width", '90px']
         ],
         "${_PREV2}": [
            ["transform", "scaleX", '0.58929'],
            ["style", "top", '82px'],
            ["transform", "scaleY", '0.57999'],
            ["style", "left", '147px']
         ],
         "${_Numero_1}": [
            ["style", "top", '30.25%'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '70px'],
            ["style", "opacity", '1'],
            ["style", "left", '17.6%'],
            ["style", "width", '90px']
         ],
         "${_HOME_butt}": [
            ["style", "top", '72px'],
            ["transform", "scaleY", '0.59344'],
            ["style", "overflow", 'hidden'],
            ["transform", "scaleX", '0.59344'],
            ["style", "left", '55px']
         ],
         "${_Numero_3}": [
            ["style", "top", '30.25%'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '70px'],
            ["style", "opacity", '1'],
            ["style", "left", '31.26%'],
            ["style", "width", '90px']
         ],
         "${_NEXT}": [
            ["transform", "scaleX", '0.58314'],
            ["style", "top", '82px'],
            ["transform", "scaleY", '0.58314'],
            ["style", "left", '1033px']
         ],
         "${_Num-6}": [
            ["style", "height", '70px'],
            ["style", "top", '72px'],
            ["style", "left", '606px'],
            ["style", "width", '90px']
         ],
         "${_Num-8}": [
            ["style", "height", '70px'],
            ["style", "top", '72px'],
            ["style", "left", '773px'],
            ["style", "width", '90px']
         ],
         "${_Numero_2}": [
            ["style", "top", '30.25%'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '70px'],
            ["style", "opacity", '1'],
            ["style", "left", '24.54%'],
            ["style", "width", '90px']
         ],
         "${symbolSelector}": [
            ["style", "height", '238px'],
            ["style", "width", '1160px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 196,
         autoPlay: false,
         timeline: [
            { id: "eid3720", tween: [ "style", "${_musica}", "left", '60px', { fromValue: '60px'}], position: 196, duration: 0, easing: "easeInOutQuad" },
            { id: "eid3731", tween: [ "style", "${_reload}", "top", '5px', { fromValue: '5px'}], position: 196, duration: 0, easing: "easeInOutQuad" },
            { id: "eid3723", tween: [ "style", "${_reload}", "left", '109px', { fromValue: '109px'}], position: 196, duration: 0, easing: "easeInOutQuad" },
            { id: "eid3728", tween: [ "style", "${_musica}", "top", '5px', { fromValue: '5px'}], position: 196, duration: 0, easing: "easeInOutQuad" }         ]
      }
   }
},
"Menu": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'MENUz',
      type: 'rect',
      rect: ['427px','-62px','auto','auto','auto','auto']
   }],
   symbolInstances: [
   {
      id: 'MENUz',
      symbolName: 'MENUz'
   }   ]
   },
   states: {
      "Base State": {
         "${_MENUz}": [
            ["style", "left", '0px'],
            ["style", "top", '0px']
         ],
         "${symbolSelector}": [
            ["style", "height", '238px'],
            ["style", "width", '1160px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 384,
         autoPlay: false,
         labels: {
            "chiusura": 223
         },
         timeline: [
            { id: "eid549", tween: [ "style", "${_MENUz}", "top", '-86px', { fromValue: '0px'}], position: 0, duration: 196, easing: "easeInOutQuad" },
            { id: "eid625", tween: [ "style", "${_MENUz}", "top", '-2px', { fromValue: '-86px'}], position: 223, duration: 161, easing: "easeInOutQuad" }         ]
      }
   }
},
"musica": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      type: 'image',
      id: 'Q0-Music',
      rect: ['0px','0px','80px','80px','auto','auto'],
      userClass: 'musicasvg',
      fill: ['rgba(0,0,0,0)','images/svg-music.svg','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${symbolSelector}": [
            ["style", "height", '80px'],
            ["style", "width", '80px']
         ],
         "${_Q0-Music}": [
            ["style", "top", '0px'],
            ["transform", "scaleY", '1'],
            ["transform", "scaleX", '1'],
            ["style", "left", '0px'],
            ["subproperty", "filter.saturate", '1']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 393,
         autoPlay: false,
         timeline: [
            { id: "eid11553", tween: [ "transform", "${_Q0-Music}", "scaleX", '0.8', { fromValue: '1'}], position: 0, duration: 223, easing: "swing" },
            { id: "eid11554", tween: [ "transform", "${_Q0-Music}", "scaleX", '1', { fromValue: '0.8'}], position: 223, duration: 170, easing: "swing" },
            { id: "eid11551", tween: [ "transform", "${_Q0-Music}", "scaleY", '0.8', { fromValue: '1'}], position: 0, duration: 223, easing: "swing" },
            { id: "eid11552", tween: [ "transform", "${_Q0-Music}", "scaleY", '1', { fromValue: '0.8'}], position: 223, duration: 170, easing: "swing" }         ]
      }
   }
},
"HOME_butt": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/G-HOME.png','0px','0px'],
      id: 'HOME',
      rect: ['0px','0px','103px','97px','auto','auto'],
      transform: [],
      boxShadow: ['inset',8,3,18,6,'rgba(0,0,0,0.648438)']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_HOME}": [
            ["subproperty", "boxShadow.inset", 'inset'],
            ["subproperty", "boxShadow.color", 'rgba(225,0,0,0.65)'],
            ["transform", "scaleX", '1'],
            ["style", "left", '0px'],
            ["style", "width", '103px'],
            ["style", "top", '0px'],
            ["transform", "scaleY", '1'],
            ["subproperty", "boxShadow.blur", '0px'],
            ["style", "height", '97px'],
            ["subproperty", "boxShadow.offsetV", '0px'],
            ["subproperty", "boxShadow.offsetH", '0px'],
            ["subproperty", "boxShadow.spread", '0px']
         ],
         "${symbolSelector}": [
            ["style", "height", '97px'],
            ["style", "width", '103px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 406,
         autoPlay: false,
         timeline: [
            { id: "eid355", tween: [ "transform", "${_HOME}", "scaleY", '0.81', { fromValue: '1'}], position: 0, duration: 218 },
            { id: "eid357", tween: [ "transform", "${_HOME}", "scaleY", '1', { fromValue: '0.81'}], position: 218, duration: 146 },
            { id: "eid354", tween: [ "transform", "${_HOME}", "scaleX", '0.81', { fromValue: '1'}], position: 0, duration: 218 },
            { id: "eid356", tween: [ "transform", "${_HOME}", "scaleX", '1', { fromValue: '0.81'}], position: 218, duration: 146 }         ]
      }
   }
},
"NEXT": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      type: 'image',
      rect: ['-8px','-11px','72px','98px','auto','auto'],
      id: 'NextImg',
      transform: [[0,0],[],[],['0.77','0.77']],
      clip: ['rect(-8px 79px 107px -14px)'],
      fill: ['rgba(0,0,0,0)','images/G-Next.png','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_NextImg}": [
            ["style", "top", '-11px'],
            ["transform", "scaleX", '1'],
            ["transform", "scaleY", '1'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '98px'],
            ["style", "left", '-8px'],
            ["style", "clip", [-8,79,107,-14], {valueTemplate:'rect(@@0@@px @@1@@px @@2@@px @@3@@px)'} ],
            ["style", "width", '72px']
         ],
         "${symbolSelector}": [
            ["style", "height", '76px'],
            ["style", "width", '56px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 140,
         autoPlay: false,
         timeline: [
            { id: "eid1367", tween: [ "transform", "${_NextImg}", "scaleX", '0.81', { fromValue: '1'}], position: 0, duration: 75 },
            { id: "eid1368", tween: [ "transform", "${_NextImg}", "scaleX", '1', { fromValue: '0.81'}], position: 75, duration: 50 },
            { id: "eid1369", tween: [ "transform", "${_NextImg}", "scaleY", '0.81', { fromValue: '1'}], position: 0, duration: 75 },
            { id: "eid1370", tween: [ "transform", "${_NextImg}", "scaleY", '1', { fromValue: '0.81'}], position: 75, duration: 50 }         ]
      }
   }
},
"PREV": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      rect: ['-8px','-11px','72px','98px','auto','auto'],
      transform: [[0,0],[],[],['0.77','0.77']],
      id: 'NextImg',
      type: 'image',
      clip: ['rect(-8px 79px 107px -14px)'],
      fill: ['rgba(0,0,0,0)','images/G-Prev.png','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_NextImg}": [
            ["style", "top", '-11px'],
            ["style", "height", '98px'],
            ["transform", "scaleY", '1'],
            ["style", "overflow", 'hidden'],
            ["transform", "scaleX", '1'],
            ["style", "clip", [-8,79,107,-14], {valueTemplate:'rect(@@0@@px @@1@@px @@2@@px @@3@@px)'} ],
            ["style", "left", '-8px'],
            ["style", "width", '72px']
         ],
         "${symbolSelector}": [
            ["style", "height", '76px'],
            ["style", "width", '56px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 140,
         autoPlay: false,
         timeline: [
            { id: "eid1367", tween: [ "transform", "${_NextImg}", "scaleX", '0.81', { fromValue: '1'}], position: 0, duration: 75 },
            { id: "eid1368", tween: [ "transform", "${_NextImg}", "scaleX", '1', { fromValue: '0.81'}], position: 75, duration: 50 },
            { id: "eid1369", tween: [ "transform", "${_NextImg}", "scaleY", '0.81', { fromValue: '1'}], position: 0, duration: 75 },
            { id: "eid1370", tween: [ "transform", "${_NextImg}", "scaleY", '1', { fromValue: '0.81'}], position: 75, duration: 50 }         ]
      }
   }
},
"testina": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'Testina',
      type: 'image',
      rect: ['0px','0px','84px','97px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/G-Testina.png','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_Testina}": [
            ["style", "top", '0px'],
            ["transform", "rotateZ", '0deg'],
            ["style", "overflow", 'hidden'],
            ["style", "height", '97px'],
            ["style", "-webkit-transform-origin", [50,91], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [50,91],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [50,91],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [50,91],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [50,91],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "left", '0px'],
            ["style", "width", '84px']
         ],
         "${symbolSelector}": [
            ["style", "height", '97px'],
            ["style", "width", '84px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 610,
         autoPlay: false,
         timeline: [
            { id: "eid5429", tween: [ "transform", "${_Testina}", "rotateZ", '21deg', { fromValue: '0deg'}], position: 0, duration: 201, easing: "easeInOutBounce" },
            { id: "eid5430", tween: [ "transform", "${_Testina}", "rotateZ", '-15deg', { fromValue: '21deg'}], position: 201, duration: 224, easing: "easeInOutBounce" },
            { id: "eid5431", tween: [ "transform", "${_Testina}", "rotateZ", '0deg', { fromValue: '-15deg'}], position: 425, duration: 185, easing: "easeInOutBounce" }         ]
      }
   }
},
"reload": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      rect: ['0px','0px','80px','80px','auto','auto'],
      id: 'reload',
      userClass: 'reload',
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/svg-reload.svg','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_reload}": [
            ["transform", "scaleX", '1'],
            ["style", "left", '0px'],
            ["transform", "scaleY", '1'],
            ["style", "top", '0px']
         ],
         "${symbolSelector}": [
            ["style", "height", '80px'],
            ["style", "width", '80px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 393,
         autoPlay: false,
         timeline: [
            { id: "eid3609", tween: [ "transform", "${_reload}", "scaleX", '0.8', { fromValue: '1'}], position: 0, duration: 223, easing: "swing" },
            { id: "eid3610", tween: [ "transform", "${_reload}", "scaleX", '1', { fromValue: '0.8'}], position: 223, duration: 170, easing: "swing" },
            { id: "eid3611", tween: [ "transform", "${_reload}", "scaleY", '0.8', { fromValue: '1'}], position: 0, duration: 223, easing: "swing" },
            { id: "eid3612", tween: [ "transform", "${_reload}", "scaleY", '1', { fromValue: '0.8'}], position: 223, duration: 170, easing: "swing" }         ]
      }
   }
},
"NUVOLA": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      userClass: 'nuvo',
      filter: [0,0,1,1,0,0,0,0,'rgba(0,0,0,0)',0,0,0],
      id: 'Nuvola',
      rect: ['0px','-10px','185px','132px','auto','auto'],
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/G-Nuvola.png','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_Nuvola}": [
            ["style", "top", '22px'],
            ["style", "height", '132px'],
            ["subproperty", "filter.drop-shadow.offsetH", '0px'],
            ["style", "left", '-180px'],
            ["style", "width", '185px']
         ],
         "${symbolSelector}": [
            ["style", "height", '132px'],
            ["style", "width", '185px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 105000,
         autoPlay: false,
         timeline: [
            { id: "eid183", tween: [ "style", "${_Nuvola}", "left", '1003px', { fromValue: '-180px'}], position: 0, duration: 105000, easing: "swing" },
            { id: "eid185", tween: [ "style", "${_Nuvola}", "top", '-83px', { fromValue: '22px'}], position: 0, duration: 105000, easing: "swing" }         ]
      }
   }
},
"Scoiattolo": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      transform: [[0,0],['-19'],[],['0.84','0.84']],
      id: 'Coda',
      type: 'image',
      rect: ['262px','-39px','231px','267px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Coda.png','0px','0px']
   },
   {
      transform: [[0,0],['5']],
      id: 'Q8-scoia-braccio1',
      type: 'image',
      rect: ['14px','163px','148px','113px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-scoia-braccio1.png','0px','0px']
   },
   {
      id: 'Q8-scoia-corpo',
      type: 'image',
      rect: ['123px','122px','212px','314px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-scoia-corpo.png','0px','0px']
   },
   {
      transform: [],
      id: 'Q8-scoia-braccio2',
      type: 'image',
      rect: ['123px','153px','99px','160px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-scoia-braccio2.png','0px','0px']
   },
   {
      transform: [[0,0],[],[],['1.15027','1.15027']],
      id: 'Q8-scoia-testa',
      type: 'image',
      rect: ['55px','-109px','206px','314px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-scoia-testa.png','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_Q8-scoia-braccio1}": [
            ["style", "-webkit-transform-origin", [91.16,24.87], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [91.16,24.87],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [91.16,24.87],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [91.16,24.87],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [91.16,24.87],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "top", '163px'],
            ["style", "left", '14px'],
            ["transform", "rotateZ", '5deg']
         ],
         "${_Q8-scoia-testa}": [
            ["style", "top", '-109px'],
            ["style", "height", '314px'],
            ["transform", "scaleY", '1.15027'],
            ["transform", "rotateZ", '0deg'],
            ["transform", "scaleX", '1.15027'],
            ["style", "-webkit-transform-origin", [51.83,81.96], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [51.83,81.96],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [51.83,81.96],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [51.83,81.96],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [51.83,81.96],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "left", '55px'],
            ["style", "width", '206px']
         ],
         "${symbolSelector}": [
            ["style", "height", '500px'],
            ["style", "width", '398px']
         ],
         "${_Q8-scoia-corpo}": [
            ["style", "left", '123px'],
            ["style", "top", '122px']
         ],
         "${_Coda}": [
            ["style", "top", '-39px'],
            ["style", "-webkit-transform-origin", [15.02,94.45], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [15.02,94.45],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [15.02,94.45],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [15.02,94.45],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [15.02,94.45],{valueTemplate:'@@0@@% @@1@@%'}],
            ["transform", "scaleY", '0.84'],
            ["transform", "rotateZ", '-19deg'],
            ["style", "height", '267px'],
            ["transform", "scaleX", '0.84'],
            ["style", "left", '262px'],
            ["style", "width", '231px']
         ],
         "${_Q8-scoia-braccio2}": [
            ["style", "top", '153px'],
            ["style", "-webkit-transform-origin", [81.5,11.78], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [81.5,11.78],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [81.5,11.78],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [81.5,11.78],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [81.5,11.78],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "left", '123px'],
            ["transform", "rotateZ", '0deg']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 2062.4506975346,
         autoPlay: false,
         labels: {
            "coda": 61,
            "ani": 750
         },
         timeline: [
            { id: "eid4336", tween: [ "transform", "${_Q8-scoia-braccio1}", "rotateZ", '55deg', { fromValue: '5deg'}], position: 750, duration: 345 },
            { id: "eid4337", tween: [ "transform", "${_Q8-scoia-braccio1}", "rotateZ", '22deg', { fromValue: '55deg'}], position: 1095, duration: 180 },
            { id: "eid4338", tween: [ "transform", "${_Q8-scoia-braccio1}", "rotateZ", '46deg', { fromValue: '22deg'}], position: 1275, duration: 131 },
            { id: "eid4339", tween: [ "transform", "${_Q8-scoia-braccio1}", "rotateZ", '20deg', { fromValue: '46deg'}], position: 1406, duration: 94 },
            { id: "eid4340", tween: [ "transform", "${_Q8-scoia-braccio1}", "rotateZ", '42deg', { fromValue: '20deg'}], position: 1500, duration: 74 },
            { id: "eid4341", tween: [ "transform", "${_Q8-scoia-braccio1}", "rotateZ", '5deg', { fromValue: '42deg'}], position: 1574, duration: 155 },
            { id: "eid4327", tween: [ "transform", "${_Q8-scoia-testa}", "rotateZ", '19deg', { fromValue: '0deg'}], position: 750, duration: 443 },
            { id: "eid4329", tween: [ "transform", "${_Q8-scoia-testa}", "rotateZ", '0deg', { fromValue: '19deg'}], position: 1500, duration: 500 },
            { id: "eid4335", tween: [ "transform", "${_Q8-scoia-braccio2}", "rotateZ", '25deg', { fromValue: '0deg'}], position: 1032, duration: 345 },
            { id: "eid4342", tween: [ "transform", "${_Q8-scoia-braccio2}", "rotateZ", '0deg', { fromValue: '25deg'}], position: 1377, duration: 479 },
            { id: "eid4323", tween: [ "transform", "${_Coda}", "rotateZ", '5deg', { fromValue: '-19deg'}], position: 61, duration: 115 },
            { id: "eid4324", tween: [ "transform", "${_Coda}", "rotateZ", '-29deg', { fromValue: '5deg'}], position: 176, duration: 208 },
            { id: "eid4325", tween: [ "transform", "${_Coda}", "rotateZ", '-19deg', { fromValue: '-29deg'}], position: 384, duration: 145 },
            { id: "eid4330", tween: [ "transform", "${_Coda}", "rotateZ", '5deg', { fromValue: '-19deg'}], position: 1000, duration: 100 },
            { id: "eid4331", tween: [ "transform", "${_Coda}", "rotateZ", '-29deg', { fromValue: '5deg'}], position: 1100, duration: 180 },
            { id: "eid4332", tween: [ "transform", "${_Coda}", "rotateZ", '-19deg', { fromValue: '-29deg'}], position: 1280, duration: 126 },
            { id: "eid4344", tween: [ "transform", "${_Coda}", "rotateZ", '5deg', { fromValue: '-19deg'}], position: 1624, duration: 68 },
            { id: "eid4345", tween: [ "transform", "${_Coda}", "rotateZ", '-29deg', { fromValue: '5deg'}], position: 1692, duration: 122 },
            { id: "eid4346", tween: [ "transform", "${_Coda}", "rotateZ", '-19deg', { fromValue: '-29deg'}], position: 1814, duration: 85 }         ]
      }
   }
},
"papera": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'Q8-papera-corpo',
      type: 'image',
      rect: ['-67px','60px','119px','79px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-papera-corpo.png','0px','0px']
   },
   {
      rect: ['-34px','-16px','135','95','auto','auto'],
      id: 'controllotesta',
      type: 'group',
      transform: [],
      c: [
      {
         type: 'image',
         id: 'Q8-papera-becco2',
         rect: ['88px','56px','29px','27px','auto','auto'],
         transform: [],
         fill: ['rgba(0,0,0,0)','images/Q8-papera-becco2.png','0px','0px']
      },
      {
         type: 'image',
         id: 'Q8-papera-becco1',
         rect: ['90px','22px','35px','35px','auto','auto'],
         transform: [[0,0],['34']],
         fill: ['rgba(0,0,0,0)','images/Q8-papera-becco1.png','0px','0px']
      },
      {
         type: 'image',
         id: 'Q8-papera-testa',
         rect: ['0px','0px','103px','95px','auto','auto'],
         transform: [],
         fill: ['rgba(0,0,0,0)','images/Q8-papera-testa.png','0px','0px']
      }]
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_Q8-papera-corpo}": [
            ["style", "left", '-67px'],
            ["style", "top", '60px']
         ],
         "${_Q8-papera-testa}": [
            ["style", "top", '0px'],
            ["style", "left", '0px'],
            ["style", "-webkit-transform-origin", [50,100], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [50,100],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [50,100],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [50,100],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [50,100],{valueTemplate:'@@0@@% @@1@@%'}]
         ],
         "${symbolSelector}": [
            ["style", "height", '79px'],
            ["style", "width", '119px']
         ],
         "${_Q8-papera-becco2}": [
            ["style", "top", '56px'],
            ["style", "-webkit-transform-origin", [0,50], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [0,50],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [0,50],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [0,50],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [0,50],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "left", '88px'],
            ["transform", "rotateZ", '0deg']
         ],
         "${_controllotesta}": [
            ["style", "-webkit-transform-origin", [28.88,96.25], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [28.88,96.25],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [28.88,96.25],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [28.88,96.25],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [28.88,96.25],{valueTemplate:'@@0@@% @@1@@%'}],
            ["transform", "rotateZ", '0deg']
         ],
         "${_Q8-papera-becco1}": [
            ["style", "-webkit-transform-origin", [18.71,76.34], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [18.71,76.34],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [18.71,76.34],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [18.71,76.34],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [18.71,76.34],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "top", '22px'],
            ["style", "left", '90px'],
            ["transform", "rotateZ", '34deg']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 5672,
         autoPlay: false,
         labels: {
            "testa": 429,
            "becco": 2500
         },
         timeline: [
            { id: "eid4415", tween: [ "transform", "${_controllotesta}", "rotateZ", '21deg', { fromValue: '0deg'}], position: 429, duration: 316 },
            { id: "eid4416", tween: [ "transform", "${_controllotesta}", "rotateZ", '-27deg', { fromValue: '21deg'}], position: 745, duration: 345 },
            { id: "eid4417", tween: [ "transform", "${_controllotesta}", "rotateZ", '6deg', { fromValue: '-27deg'}], position: 1091, duration: 211 },
            { id: "eid4418", tween: [ "transform", "${_controllotesta}", "rotateZ", '0deg', { fromValue: '6deg'}], position: 1302, duration: 284 },
            { id: "eid4421", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '-9deg', { fromValue: '0deg'}], position: 745, duration: 326 },
            { id: "eid4423", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '26deg', { fromValue: '-9deg'}], position: 1072, duration: 259 },
            { id: "eid4425", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '0deg', { fromValue: '26deg'}], position: 1330, duration: 353 },
            { id: "eid4429", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '25deg', { fromValue: '0deg'}], position: 2500, duration: 240 },
            { id: "eid4431", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '-11deg', { fromValue: '25deg'}], position: 2740, duration: 288 },
            { id: "eid4435", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '25deg', { fromValue: '0deg'}], position: 3029, duration: 240 },
            { id: "eid4436", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '-11deg', { fromValue: '25deg'}], position: 3269, duration: 288 },
            { id: "eid4439", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '25deg', { fromValue: '0deg'}], position: 3557, duration: 240 },
            { id: "eid4440", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '-11deg', { fromValue: '25deg'}], position: 3798, duration: 288 },
            { id: "eid4443", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '25deg', { fromValue: '0deg'}], position: 4086, duration: 240 },
            { id: "eid4444", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '-11deg', { fromValue: '25deg'}], position: 4326, duration: 288 },
            { id: "eid4447", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '25deg', { fromValue: '0deg'}], position: 4615, duration: 240 },
            { id: "eid4448", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '-11deg', { fromValue: '25deg'}], position: 4855, duration: 288 },
            { id: "eid4451", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '25deg', { fromValue: '0deg'}], position: 5143, duration: 240 },
            { id: "eid4452", tween: [ "transform", "${_Q8-papera-becco2}", "rotateZ", '-11deg', { fromValue: '25deg'}], position: 5384, duration: 288 },
            { id: "eid4422", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '43deg', { fromValue: '34deg'}], position: 745, duration: 326 },
            { id: "eid4424", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '3deg', { fromValue: '43deg'}], position: 1072, duration: 259 },
            { id: "eid4426", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '34deg', { fromValue: '3deg'}], position: 1330, duration: 353 },
            { id: "eid4430", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '11deg', { fromValue: '34deg'}], position: 2500, duration: 240 },
            { id: "eid4432", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '46deg', { fromValue: '11deg'}], position: 2740, duration: 288 },
            { id: "eid4433", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '11deg', { fromValue: '34deg'}], position: 3029, duration: 240 },
            { id: "eid4434", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '46deg', { fromValue: '11deg'}], position: 3269, duration: 288 },
            { id: "eid4437", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '11deg', { fromValue: '34deg'}], position: 3557, duration: 240 },
            { id: "eid4438", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '46deg', { fromValue: '11deg'}], position: 3798, duration: 288 },
            { id: "eid4441", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '11deg', { fromValue: '34deg'}], position: 4086, duration: 240 },
            { id: "eid4442", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '46deg', { fromValue: '11deg'}], position: 4326, duration: 288 },
            { id: "eid4445", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '11deg', { fromValue: '34deg'}], position: 4615, duration: 240 },
            { id: "eid4446", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '46deg', { fromValue: '11deg'}], position: 4855, duration: 288 },
            { id: "eid4449", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '11deg', { fromValue: '34deg'}], position: 5143, duration: 240 },
            { id: "eid4450", tween: [ "transform", "${_Q8-papera-becco1}", "rotateZ", '46deg', { fromValue: '11deg'}], position: 5384, duration: 288 }         ]
      }
   }
},
"giunco": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      rect: ['0px','0px','89px','500px','auto','auto'],
      id: 'Q8-giunco',
      transform: [],
      type: 'image',
      fill: ['rgba(0,0,0,0)','images/Q8-giunco.png','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_Q8-giunco}": [
            ["style", "-webkit-transform-origin", [17.85,97.48], {valueTemplate:'@@0@@% @@1@@%'} ],
            ["style", "-moz-transform-origin", [17.85,97.48],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-ms-transform-origin", [17.85,97.48],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "msTransformOrigin", [17.85,97.48],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "-o-transform-origin", [17.85,97.48],{valueTemplate:'@@0@@% @@1@@%'}],
            ["style", "top", '0px'],
            ["style", "left", '0px'],
            ["transform", "rotateZ", '0deg']
         ],
         "${symbolSelector}": [
            ["style", "height", '500px'],
            ["style", "width", '89px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 7750,
         autoPlay: false,
         labels: {
            "normale": 351,
            "breve": 4420
         },
         timeline: [
            { id: "eid4499", tween: [ "transform", "${_Q8-giunco}", "rotateZ", '3deg', { fromValue: '0deg'}], position: 351, duration: 696, easing: "easeInOutQuad" },
            { id: "eid4500", tween: [ "transform", "${_Q8-giunco}", "rotateZ", '-8deg', { fromValue: '3deg'}], position: 1047, duration: 1554, easing: "easeInOutQuad" },
            { id: "eid4501", tween: [ "transform", "${_Q8-giunco}", "rotateZ", '0deg', { fromValue: '-8deg'}], position: 2601, duration: 1500, easing: "easeInOutQuad" },
            { id: "eid4503", tween: [ "transform", "${_Q8-giunco}", "rotateZ", '5deg', { fromValue: '0deg'}], position: 4420, duration: 1702, easing: "easeInOutQuad" },
            { id: "eid4504", tween: [ "transform", "${_Q8-giunco}", "rotateZ", '0deg', { fromValue: '5deg'}], position: 6122, duration: 1628, easing: "easeInOutQuad" }         ]
      }
   }
},
"paperatutto1": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'papera1',
      type: 'rect',
      rect: ['-30px','-20px','auto','auto','auto','auto'],
      transform: [[0,0],[],[],['0.4958','0.4958']]
   }],
   symbolInstances: [
   {
      id: 'papera1',
      symbolName: 'papera'
   }   ]
   },
   states: {
      "Base State": {
         "${symbolSelector}": [
            ["style", "height", '39px'],
            ["style", "width", '59px']
         ],
         "${_papera1}": [
            ["transform", "scaleX", '0.4958'],
            ["transform", "scaleY", '0.4958'],
            ["motion", "location", '29.500174816744px 19.584110966435px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4500,
         autoPlay: false,
         timeline: [
            { id: "eid4409", tween: [ "motion", "${_papera1}", [[29.5,19.58,0,0],[189.5,26.66,241.86,21.45]]], position: 0, duration: 1068 },
            { id: "eid4411", tween: [ "motion", "${_papera1}", [[189.5,26.66,105.48,9.36],[284.5,11.58,101.35,5.51]]], position: 1068, duration: 815 },
            { id: "eid4412", tween: [ "motion", "${_papera1}", [[284.5,11.58,96.7,5.26],[388.5,40.66,95.29,-31.92]]], position: 1883, duration: 762 },
            { id: "eid4410", tween: [ "motion", "${_papera1}", [[388.5,40.66,82.34,-27.59],[535.5,38.66,174.59,-24.45,161.12,-22.56],[608.44,0.5,0,0]]], position: 2644, duration: 1856 }         ]
      }
   }
},
"paperatutto2": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'papera2',
      type: 'rect',
      rect: ['-30px','-20px','auto','auto','auto','auto'],
      transform: [[0,0],[],[],['0.4958','0.4958']]
   }],
   symbolInstances: [
   {
      id: 'papera2',
      symbolName: 'papera'
   }   ]
   },
   states: {
      "Base State": {
         "${_papera2}": [
            ["transform", "scaleX", '0.4958'],
            ["transform", "scaleY", '0.4958'],
            ["motion", "location", '29.500174816744px 19.584110966435px']
         ],
         "${symbolSelector}": [
            ["style", "height", '39px'],
            ["style", "width", '59px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4500,
         autoPlay: false,
         timeline: [
            { id: "eid4464", tween: [ "motion", "${_papera2}", [[29.5,19.58,0,0],[189.5,26.66,241.86,21.45]]], position: 0, duration: 1068 },
            { id: "eid4465", tween: [ "motion", "${_papera2}", [[189.5,26.66,105.48,9.36],[284.5,11.58,101.35,5.51]]], position: 1068, duration: 815 },
            { id: "eid4466", tween: [ "motion", "${_papera2}", [[284.5,11.58,96.7,5.26],[388.5,40.66,97.68,23.61]]], position: 1883, duration: 762 },
            { id: "eid4467", tween: [ "motion", "${_papera2}", [[388.5,40.66,91.69,22.16],[484.8,72.06,174.59,35.77,159.38,32.65],[591.5,79.66,0,0]]], position: 2644, duration: 1856 }         ]
      }
   }
},
"paperatutto4": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'papera4',
      type: 'rect',
      rect: ['-30px','-20px','auto','auto','auto','auto'],
      transform: [[0,0],[],[],['0.4958','0.4958']]
   }],
   symbolInstances: [
   {
      id: 'papera4',
      symbolName: 'papera'
   }   ]
   },
   states: {
      "Base State": {
         "${_papera4}": [
            ["transform", "scaleX", '0.4958'],
            ["transform", "scaleY", '0.4958'],
            ["motion", "location", '29.500174816744px 19.584110966435px']
         ],
         "${symbolSelector}": [
            ["style", "height", '39px'],
            ["style", "width", '59px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4500,
         autoPlay: false,
         timeline: [
            { id: "eid4472", tween: [ "motion", "${_papera4}", [[29.5,19.58,0,0],[83.5,15.66,91.1,2.47,81.63,2.21],[189.5,26.66,127.56,11.31]]], position: 0, duration: 1068 },
            { id: "eid4473", tween: [ "motion", "${_papera4}", [[189.5,26.66,105.48,9.36],[284.5,11.58,101.35,5.51]]], position: 1068, duration: 815 },
            { id: "eid4474", tween: [ "motion", "${_papera4}", [[284.5,11.58,96.7,5.26],[402.5,54.5,100.33,-5.68]]], position: 1883, duration: 762 },
            { id: "eid4475", tween: [ "motion", "${_papera4}", [[402.5,54.5,104.58,-5.92],[461.76,-34.18,66.17,-110.45,64.64,-107.9],[556.5,-139.15,0,0]]], position: 2644, duration: 1856 }         ]
      }
   }
},
"paperatutto8": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'papera8',
      type: 'rect',
      rect: ['-30px','-20px','auto','auto','auto','auto'],
      transform: [[0,0],[],[],['0.4958','0.4958']]
   }],
   symbolInstances: [
   {
      id: 'papera8',
      symbolName: 'papera'
   }   ]
   },
   states: {
      "Base State": {
         "${symbolSelector}": [
            ["style", "height", '39px'],
            ["style", "width", '59px']
         ],
         "${_papera8}": [
            ["transform", "scaleX", '0.4958'],
            ["transform", "scaleY", '0.4958'],
            ["motion", "location", '29.500174816744px 19.584110966435px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4500,
         autoPlay: false,
         timeline: [
            { id: "eid4489", tween: [ "motion", "${_papera8}", [[29.5,19.58,0,0],[189.5,26.66,241.86,21.45]]], position: 0, duration: 1068 },
            { id: "eid4490", tween: [ "motion", "${_papera8}", [[189.5,26.66,105.48,9.36],[284.5,11.58,101.35,5.51]]], position: 1068, duration: 815 },
            { id: "eid4491", tween: [ "motion", "${_papera8}", [[284.5,11.58,96.7,5.26],[335.68,21.85,45.42,16.22]]], position: 1883, duration: 762 },
            { id: "eid4492", tween: [ "motion", "${_papera8}", [[335.68,21.85,36.23,12.93],[379.39,37.08,104.45,4.79,27.64,1.27],[442.93,66.84,54.43,22.1,73.45,29.83],[487.11,77.95,0,0]]], position: 2644, duration: 1856 }         ]
      }
   }
},
"paperatutto6": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'papera6',
      type: 'rect',
      rect: ['-30px','-20px','auto','auto','auto','auto'],
      transform: [[0,0],[],[],['0.4958','0.4958']]
   }],
   symbolInstances: [
   {
      id: 'papera6',
      symbolName: 'papera'
   }   ]
   },
   states: {
      "Base State": {
         "${symbolSelector}": [
            ["style", "height", '39px'],
            ["style", "width", '59px']
         ],
         "${_papera6}": [
            ["transform", "scaleX", '0.4958'],
            ["transform", "scaleY", '0.4958'],
            ["motion", "location", '29.500174816744px 19.584110966435px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4500,
         autoPlay: false,
         timeline: [
            { id: "eid4481", tween: [ "motion", "${_papera6}", [[29.5,19.58,0,0],[166.5,19.66,320.29,25.82]]], position: 0, duration: 1068 },
            { id: "eid4482", tween: [ "motion", "${_papera6}", [[166.5,19.66,123.33,20.1],[270.92,28.75,101.35,5.51]]], position: 1068, duration: 815 },
            { id: "eid4483", tween: [ "motion", "${_papera6}", [[270.92,28.75,96.7,5.26],[359.03,-19.58,72.53,-34.95]]], position: 1883, duration: 762 },
            { id: "eid4484", tween: [ "motion", "${_papera6}", [[359.03,-19.58,111.93,-53.94],[444.13,-54.81,127.14,-20.34,78.21,-12.51],[487.41,-61.34,0,0]]], position: 2644, duration: 1856 }         ]
      }
   }
},
"paperatutto3": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'papera3',
      type: 'rect',
      rect: ['-30px','-20px','auto','auto','auto','auto'],
      transform: [[0,0],[],[],['0.4958','0.4958']]
   }],
   symbolInstances: [
   {
      id: 'papera3',
      symbolName: 'papera'
   }   ]
   },
   states: {
      "Base State": {
         "${symbolSelector}": [
            ["style", "height", '39px'],
            ["style", "width", '59px']
         ],
         "${_papera3}": [
            ["transform", "scaleX", '0.4958'],
            ["transform", "scaleY", '0.4958'],
            ["motion", "location", '29.500174816744px 19.584110966435px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4500,
         autoPlay: false,
         timeline: [
            { id: "eid4468", tween: [ "motion", "${_papera3}", [[29.5,19.58,0,0],[189.5,26.66,241.86,21.45]]], position: 0, duration: 1068 },
            { id: "eid4469", tween: [ "motion", "${_papera3}", [[189.5,26.66,105.48,9.36],[284.5,11.58,101.35,5.51]]], position: 1068, duration: 815 },
            { id: "eid4470", tween: [ "motion", "${_papera3}", [[284.5,11.58,96.7,5.26],[388.5,34.5,95.29,-31.92]]], position: 1883, duration: 762 },
            { id: "eid4471", tween: [ "motion", "${_papera3}", [[388.5,34.5,82.34,-27.59],[492.76,4.46,126.27,-65.93,141.1,-73.67],[584.79,-69.34,0,0]]], position: 2644, duration: 1856 }         ]
      }
   }
},
"paperatutto5": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'papera5',
      type: 'rect',
      transform: [[0,0],[],[],['0.4958','0.4958']],
      rect: ['-30px','-20px','auto','auto','auto','auto']
   }],
   symbolInstances: [
   {
      id: 'papera5',
      symbolName: 'papera'
   }   ]
   },
   states: {
      "Base State": {
         "${symbolSelector}": [
            ["style", "height", '39px'],
            ["style", "width", '59px']
         ],
         "${_papera5}": [
            ["transform", "scaleX", '0.4958'],
            ["transform", "scaleY", '0.4958'],
            ["motion", "location", '29.500174816744px 19.584110966435px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4500,
         autoPlay: false,
         timeline: [
            { id: "eid4476", tween: [ "motion", "${_papera5}", [[29.5,19.58,0,0],[189.5,26.66,241.86,21.45]]], position: 0, duration: 1068 },
            { id: "eid4477", tween: [ "motion", "${_papera5}", [[189.5,26.66,105.48,9.36],[284.5,11.58,101.35,-36.67]]], position: 1068, duration: 815 },
            { id: "eid4478", tween: [ "motion", "${_papera5}", [[284.5,11.58,91.07,-32.95],[382.5,-19.5,98.69,-29.31]]], position: 1883, duration: 762 },
            { id: "eid4479", tween: [ "motion", "${_papera5}", [[382.5,-19.5,94.26,-27.99],[449.71,-53.18,52.29,-65.54,29.67,-37.2],[467.5,-134.34,0,0]]], position: 2644, duration: 1856 }         ]
      }
   }
},
"paperatutto7": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'papera7',
      type: 'rect',
      transform: [[0,0],[],[],['0.4958','0.4958']],
      rect: ['-30px','-20px','auto','auto','auto','auto']
   }],
   symbolInstances: [
   {
      id: 'papera7',
      symbolName: 'papera'
   }   ]
   },
   states: {
      "Base State": {
         "${_papera7}": [
            ["transform", "scaleX", '0.4958'],
            ["transform", "scaleY", '0.4958'],
            ["motion", "location", '29.500174816744px 19.584110966435px']
         ],
         "${symbolSelector}": [
            ["style", "height", '39px'],
            ["style", "width", '59px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4500,
         autoPlay: false,
         timeline: [
            { id: "eid4485", tween: [ "motion", "${_papera7}", [[29.5,19.58,0,0],[189.5,26.66,241.86,21.45]]], position: 0, duration: 1068 },
            { id: "eid4486", tween: [ "motion", "${_papera7}", [[189.5,26.66,105.48,9.36],[284.5,11.58,98.31,-25.23]]], position: 1068, duration: 815 },
            { id: "eid4487", tween: [ "motion", "${_papera7}", [[284.5,11.58,93.67,-24.04],[402.5,-27.34,94.9,33.06]]], position: 1883, duration: 762 },
            { id: "eid4488", tween: [ "motion", "${_papera7}", [[402.5,-27.34,92.85,32.35],[444.23,3.44,95.6,23.55,57.21,14.09],[519.5,11.6,0,0]]], position: 2644, duration: 1856 }         ]
      }
   }
},
"fronde3WATER": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'Q8-fronde3WATER',
      type: 'image',
      rect: ['0px','0px','82px','24px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-fronde2WATER.png','0px','0px']
   },
   {
      id: 'Q8-fronde3WATERCopy7',
      type: 'image',
      rect: ['0px','0px','82px','24px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-fronde2WATER.png','0px','0px']
   },
   {
      id: 'Q8-fronde3WATERCopy8',
      type: 'image',
      rect: ['0px','0px','82px','24px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-fronde2WATER.png','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_Q8-fronde3WATERCopy8}": [
            ["style", "top", '-48px'],
            ["transform", "scaleY", '1'],
            ["transform", "scaleX", '1'],
            ["style", "opacity", '1'],
            ["style", "left", '13px']
         ],
         "${symbolSelector}": [
            ["style", "height", '24px'],
            ["style", "width", '82px']
         ],
         "${_Q8-fronde3WATER}": [
            ["style", "top", '-48px'],
            ["transform", "scaleY", '1'],
            ["transform", "scaleX", '1'],
            ["style", "opacity", '1'],
            ["style", "left", '13px']
         ],
         "${_Q8-fronde3WATERCopy7}": [
            ["style", "top", '-48px'],
            ["transform", "scaleY", '1'],
            ["transform", "scaleX", '1'],
            ["style", "opacity", '1'],
            ["style", "left", '13px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4116.6898258449,
         autoPlay: false,
         timeline: [
            { id: "eid4622", tween: [ "style", "${_Q8-fronde3WATERCopy8}", "left", '9px', { fromValue: '13px'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4624", tween: [ "style", "${_Q8-fronde3WATERCopy8}", "left", '13px', { fromValue: '9px'}], position: 3500, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4614", tween: [ "transform", "${_Q8-fronde3WATERCopy7}", "scaleY", '1.24', { fromValue: '1'}], position: 500, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4621", tween: [ "style", "${_Q8-fronde3WATERCopy8}", "opacity", '0', { fromValue: '1'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4623", tween: [ "style", "${_Q8-fronde3WATERCopy8}", "opacity", '1', { fromValue: '0'}], position: 3500, duration: 500, easing: "easeInOutQuad" },
            { id: "eid4620", tween: [ "transform", "${_Q8-fronde3WATERCopy8}", "scaleX", '1.24', { fromValue: '1'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4626", tween: [ "transform", "${_Q8-fronde3WATERCopy8}", "scaleX", '1', { fromValue: '1.24'}], position: 3500, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4523", tween: [ "transform", "${_Q8-fronde3WATER}", "scaleX", '1.24', { fromValue: '1'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4615", tween: [ "transform", "${_Q8-fronde3WATERCopy7}", "scaleX", '1.24', { fromValue: '1'}], position: 500, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4525", tween: [ "style", "${_Q8-fronde3WATER}", "left", '9px', { fromValue: '13px'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4619", tween: [ "transform", "${_Q8-fronde3WATERCopy8}", "scaleY", '1.24', { fromValue: '1'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4627", tween: [ "transform", "${_Q8-fronde3WATERCopy8}", "scaleY", '1', { fromValue: '1.24'}], position: 3500, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4617", tween: [ "style", "${_Q8-fronde3WATERCopy7}", "left", '9px', { fromValue: '13px'}], position: 500, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4618", tween: [ "style", "${_Q8-fronde3WATERCopy8}", "top", '-21px', { fromValue: '-48px'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4625", tween: [ "style", "${_Q8-fronde3WATERCopy8}", "top", '-48px', { fromValue: '-21px'}], position: 3500, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4521", tween: [ "style", "${_Q8-fronde3WATER}", "top", '-21px', { fromValue: '-48px'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4524", tween: [ "style", "${_Q8-fronde3WATER}", "opacity", '0', { fromValue: '1'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4613", tween: [ "style", "${_Q8-fronde3WATERCopy7}", "top", '-21px', { fromValue: '-48px'}], position: 500, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4616", tween: [ "style", "${_Q8-fronde3WATERCopy7}", "opacity", '0', { fromValue: '1'}], position: 500, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4522", tween: [ "transform", "${_Q8-fronde3WATER}", "scaleY", '1.24', { fromValue: '1'}], position: 0, duration: 1750, easing: "easeInOutQuad" }         ]
      }
   }
},
"fronde2WATER": {
   version: "2.0.1",
   minimumCompatibleVersion: "2.0.0",
   build: "2.0.1.268",
   baseState: "Base State",
   initialState: "Base State",
   gpuAccelerate: false,
   resizeInstances: false,
   content: {
   dom: [
   {
      id: 'Q8-fronde3WATER',
      type: 'image',
      rect: ['0px','0px','82px','24px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-fronde3WATER.png','0px','0px']
   },
   {
      id: 'Q8-fronde3WATERCopy5',
      type: 'image',
      rect: ['0px','0px','82px','24px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-fronde3WATER.png','0px','0px']
   },
   {
      id: 'Q8-fronde3WATERCopy6',
      type: 'image',
      rect: ['0px','0px','82px','24px','auto','auto'],
      fill: ['rgba(0,0,0,0)','images/Q8-fronde3WATER.png','0px','0px']
   }],
   symbolInstances: [
   ]
   },
   states: {
      "Base State": {
         "${_Q8-fronde3WATERCopy6}": [
            ["style", "top", '-48px'],
            ["transform", "scaleY", '1'],
            ["transform", "scaleX", '1'],
            ["style", "opacity", '1'],
            ["style", "left", '13px']
         ],
         "${_Q8-fronde3WATERCopy5}": [
            ["style", "top", '-48px'],
            ["transform", "scaleY", '1'],
            ["transform", "scaleX", '1'],
            ["style", "opacity", '1'],
            ["style", "left", '13px']
         ],
         "${symbolSelector}": [
            ["style", "height", '24px'],
            ["style", "width", '82px']
         ],
         "${_Q8-fronde3WATER}": [
            ["style", "top", '-48px'],
            ["transform", "scaleY", '1'],
            ["transform", "scaleX", '1'],
            ["style", "opacity", '1'],
            ["style", "left", '13px']
         ]
      }
   },
   timelines: {
      "Default Timeline": {
         fromState: "Base State",
         toState: "",
         duration: 4000,
         autoPlay: false,
         timeline: [
            { id: "eid4602", tween: [ "style", "${_Q8-fronde3WATERCopy6}", "left", '8px', { fromValue: '13px'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4609", tween: [ "style", "${_Q8-fronde3WATERCopy6}", "left", '13px', { fromValue: '8px'}], position: 3500, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4594", tween: [ "transform", "${_Q8-fronde3WATERCopy5}", "scaleY", '1.24', { fromValue: '1'}], position: 628, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4600", tween: [ "transform", "${_Q8-fronde3WATERCopy6}", "scaleX", '1.24', { fromValue: '1'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4611", tween: [ "transform", "${_Q8-fronde3WATERCopy6}", "scaleX", '1', { fromValue: '1.24'}], position: 3500, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4596", tween: [ "style", "${_Q8-fronde3WATERCopy5}", "opacity", '0', { fromValue: '1'}], position: 628, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4523", tween: [ "transform", "${_Q8-fronde3WATER}", "scaleX", '1.24', { fromValue: '1'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4595", tween: [ "transform", "${_Q8-fronde3WATERCopy5}", "scaleX", '1.24', { fromValue: '1'}], position: 628, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4525", tween: [ "style", "${_Q8-fronde3WATER}", "left", '8px', { fromValue: '13px'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4598", tween: [ "style", "${_Q8-fronde3WATERCopy6}", "top", '-27px', { fromValue: '-48px'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4610", tween: [ "style", "${_Q8-fronde3WATERCopy6}", "top", '-48px', { fromValue: '-27px'}], position: 3500, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4601", tween: [ "style", "${_Q8-fronde3WATERCopy6}", "opacity", '0', { fromValue: '1'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4608", tween: [ "style", "${_Q8-fronde3WATERCopy6}", "opacity", '1', { fromValue: '0'}], position: 3500, duration: 500, easing: "easeInOutQuad" },
            { id: "eid4599", tween: [ "transform", "${_Q8-fronde3WATERCopy6}", "scaleY", '1.24', { fromValue: '1'}], position: 1250, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4612", tween: [ "transform", "${_Q8-fronde3WATERCopy6}", "scaleY", '1', { fromValue: '1.24'}], position: 3500, duration: 0, easing: "easeInOutQuad" },
            { id: "eid4521", tween: [ "style", "${_Q8-fronde3WATER}", "top", '-27px', { fromValue: '-48px'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4524", tween: [ "style", "${_Q8-fronde3WATER}", "opacity", '0', { fromValue: '1'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4597", tween: [ "style", "${_Q8-fronde3WATERCopy5}", "left", '8px', { fromValue: '13px'}], position: 628, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4522", tween: [ "transform", "${_Q8-fronde3WATER}", "scaleY", '1.24', { fromValue: '1'}], position: 0, duration: 1750, easing: "easeInOutQuad" },
            { id: "eid4593", tween: [ "style", "${_Q8-fronde3WATERCopy5}", "top", '-27px', { fromValue: '-48px'}], position: 628, duration: 1750, easing: "easeInOutQuad" }         ]
      }
   }
}
};


Edge.registerCompositionDefn(compId, symbols, fonts, resources);

/**
 * Adobe Edge DOM Ready Event Handler
 */
$(window).ready(function() {
     Edge.launchComposition(compId);
});
})(jQuery, AdobeEdge, "Quadro-08");
